...
for ip = iProfRun
  ...
  if iDoJac(1) > 0
    [radsOut,jacsOut] = matlab_kcarta_uplook_jac(head,prof,aux_struct,...
                                             ropt0,iDoJac,iJacobOutput);
  else
    [radsOut] = matlab_kcarta_uplook_nojac(head,prof,aux_struct,ropt0);
    end
  end
...
clear fA fB dirin fin
clear h p iDoJac iJacobOutput;
