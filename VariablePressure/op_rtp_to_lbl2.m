...
if (~isfield(head,'glist'))
   ...
   if (length(gasid) ~= ngas)
      ...
      [iaT,adjustor] = adjust100layers_to_Nlayers(prof);
      for ii = 1 : length(I)
        if C(ii) ~= 103
        ...
        elseif C(ii) == 103
          ...
          head.ngas = head.ngas + 1;
          head.glist(head.ngas) = C(ii); head.gunit(head.ngas) = 1;
          str = ['prof.gas_103 = prof.gas_1 * ' num2str(mult103) ';']; %% no need to check further
          eval(str);
          end
        end
     [igas,indglist,indgasid] = intersect(head.glist,gasid);
