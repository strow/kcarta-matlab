function [hout,pout,iWarning,fout,rout] = driver_process_kcarta_rtp(hin,hain,pin,pain,profiles,iInstr)
...
w    = radsOut.freqAllChunks;
dall = monorad';
if iInstr == 1
  disp('AIRS convolution, all channels')
  sfile = '/asl/matlab2012/srftest/srftables_m140f_withfake_mar08.hdf';
  [rout, fout] = sconv2(dall, w, 1:2378, sfile);

elseif iInstr == 2
  disp('IASI convolution, all channels')

  [rKcIasi2, fiasi2] = kc2iasi(dall, w);
...
elseif iInstr == 3
  ...
  [rad1, frq1] = kc2cris(user, dall, w);

  band = 'MW';   % cris band
  [inst, user] = inst_params(band, wlaser);
  [rad2, frq2] = kc2cris(user, dall, w);

  band = 'SW';   % cris band
  [inst, user] = inst_params(band, wlaser);
  [rad3, frq3] = kc2cris(user, dall, w);
...
elseif iInstr == 4
  ...
  band = 'LW';   % cris band
  [inst, user] = inst_params(band, wlaser);
  [hrad1, hfrq1] = kc2cris(user, dall, w);

  band = 'MW';   % cris band
  [inst, user] = inst_params(band, wlaser,opt);
  [hrad2, hfrq2] = kc2cris(user, dall, w);

  band = 'SW';   % cris band
  [inst, user] = inst_params(band, wlaser,opt);
  [hrad3, hfrq3] = kc2cris(user, dall, w);
